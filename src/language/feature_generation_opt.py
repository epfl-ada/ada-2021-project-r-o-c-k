'''
File name: feature_generation_opt.py
Author: Oskar 
...
def get_sentiment(quote):
    '''
    Returns sentiment, based on NLTK's polarity score
    -1 one means 100 % negative, 1 100 % positive
    :param quote: quote to analyze
    :return: sentiment
    '''
    sentiments = sid.polarity_scores(quote)
...
def add_morphological_cols(row):
    ...
    row['sentiment'] = get_sentiment(quote)
...
def add_features_to_df(df):
    ...
    new_df = new_df.mapply(add_morphological_cols, axis = 1, n_workers = 8)
    #new_df = new_df.mapply(add_sentiment_cols, axis = 1, n_workers = 8)
